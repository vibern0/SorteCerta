...
export async function readProtocolSnapshotAtBlock(
  ...
  const [
    ...
    read(snapshotClient, deployment.morpho, morphoBlueAbi, "idToMarketParams", [
      deployment.marketId,
    ]),
    read(snapshotClient, deployment.morpho, morphoBlueAbi, "market", [
      deployment.marketId,
    ]),
  ]);
  ...
  const [
    ...
    readWithdrawalBatch(
      snapshotClient,
      deployment.pool,
      asBigInt(withdrawalBatchId, "withdrawal batch id"),
    ),
    read(snapshotClient, marketParams.oracle, morphoOracleAbi, "price").then(
      (value) => asBigInt(value, "oracle price"),
    ),
    readBorrowRate(snapshotClient, marketParams, marketState),
    read(snapshotClient, deployment.usdc, erc20Abi, "balanceOf", [
      deployment.adapter,
    ]),
    read(snapshotClient, deployment.morpho, morphoBlueAbi, "position", [
      deployment.marketId,
      deployment.adapter,
    ]),
  ]);
  ...
  const rawAccruedYieldAssets = asBigInt(
    accruedYieldAssets,
    "accrued yield assets",
  );
  ...
  return {
    ...
    pool: {
      ...
      morphoPendingDepositCount: asBigInt(
        morphoPendingDepositCount,
    ...
    adapter: {
      ...
      availablePrincipalAssets: asBigInt(
        availablePrincipalAssets,
...
async function readWithdrawalBatch(
  ...
  const [
    ...
    read(client, pool, confidentialPrizePoolAbi, "withdrawalBatchClosesAt", [
      id,
    ]),
...
function readBorrowRate(
  ...
  return Promise.resolve()
    .then(() =>
      read(client, marketParams.irm, morphoIrmAbi, "borrowRateView", [
        marketParams,
        marketState,
      ]),
    )
...
async function readAccount(
  ...
  const [
    ...
    read(client, deployment.morpho, morphoBlueAbi, "position", [
      deployment.marketId,
      account,
    ]),
    client.getBalance({ address: account, blockNumber: client.blockNumber }),
    read(client, deployment.weth, erc20Abi, "balanceOf", [account]),
    read(client, deployment.usdc, erc20Abi, "allowance", [
      account,
      deployment.morpho,
    ]),
    read(client, deployment.weth, erc20Abi, "allowance", [
      account,
      deployment.morpho,
    ]),
  ...
  return {
    ...
    tokens: {
      ...
      morphoUsdcAllowance: asBigInt(
        morphoUsdcAllowance,
        "Morpho USDC allowance",
      ),
      morphoWethAllowance: asBigInt(
        morphoWethAllowance,
        "Morpho WETH allowance",
      ),
